...
void 
copysmonom (s_monom src, s_monom * dst)
{
  vmrec rec;
  vmptr vs, vd;
...
static void 
reducec (NUM_TYPE * l1, NUM_TYPE * l2)
{
  NUM_TYPE c, i1, i2;
...
static void 
comparenames (char *s1, char *s2, int *eq, int *gt)
{
  int i, l1, l2, n1, n2;
...
void 
sew_vm (vmptr * p1, vmptr p2, int mlt)
{
  vmptr m, mm, m1, m2;
  int gt, eq;
  vmrec mrec;
...
void 
reduce_s (s_monom * s1, s_monom * s2)
{
  reducec (&(s1->c), &(s2->c));
  reducev (&s1->v, &s2->v);
}
...
void 
mult_s (s_monom * s1, s_monom * s2)
{
  s1->c *= s2->c;
  sew_vm (&s1->v, s2->v, TRUE);
  delsqrt2 (s1);
  del_i (s1);
}

void 
mult_rptr (rmptr * m1, rmptr * m2)
{
  reduce_s (&(*m1)->n, &(*m2)->d);
  reduce_s (&(*m1)->d, &(*m2)->n);
...
static void *
bact_r (char ch, void *mm1, void *mm2)
{
  rmptr m1, m2;
  int i;
  NUM_TYPE ln, ld, d;
...
static void *
act_rat (char *name, int n, void **args)
{
  if (n == 1)
...
void 
diagramsrfactors (hlpcsptr gst, s_listptr * s, rmptr * totf)
{
