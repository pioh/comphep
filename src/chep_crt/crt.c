...
static int bgi_x = 0;
static int bgi_y = 0;
static int bgi_maxx = 639;
static int bgi_maxy = 479;
static int CdX = 8;
static int CdY = 19;

static int bgi_hj = LeftText;
static int bgi_vj = TopText;
...
static void
texline (int x1, int y1, int x2, int y2, int arrowflag)
{
  ...
  linescale = sqrt( (x-fx)*(x-fx) + (y-fy)*(y-fy) )/10.0; 
...
static void
texouttext (int x, int y, char *txt)
{
...
maxCol (void)
{
  return MIN ((bgi_maxx + 1) / CdX, STRSIZ - 1);
}

int
maxRow (void)
{
  return (bgi_maxy + 1) / CdY;
}

void
tg_gettextsettings (struct tg_textsettingstype *txi)
{
  txi->horiz = bgi_hj;
  txi->vert = bgi_vj;
}
...
void
tg_getviewsettings (struct tg_viewporttype *vp)
{
...
void
tg_setviewport (int left, int top, int right, int bottom)
{
  bgi_vport.left = left;
  bgi_vport.top = top;
...
void
tg_setlinestyle (int linestyle, int thickness)
{
...
static void
addToGraphList (int type, int x1, int y1, int x2, int y2,
		int color, int aux, char *text)
{
  ...
  while (p)
	...
	case t_xt:
	  ...
	  break;
	case line:
	  del = p->type == line && p->x1 == x1 && p->y1 == y1
	    && p->x2 == x2 && p->y2 == y2;
	  break;
	case string:
...
static void
repeatGrCom (int x1, int y1, int x2, int y2, int all)
{
  ...
  while (p != NULL)
	...
	switch (p->type)
	  ...
	  case bar:
	    ...
	    break;
	  case line:
	    sg_drawLine (p->x1, p->y1, p->x2, p->y2, p->color,
			 p->aux & 15, p->aux / 16);
	    break;
	  case string:
...
static void
crt_expose (int x1, int y1, int w, int h)
{
  repeatGrCom (x1, y1, x1 + w, y1 + h, TRUE);
  refresh_scr ();
}

void
tg_bar (int left, int top, int right, int bottom)
{
  int x1, x2, y1, y2;
...
void
tg_line (int x1, int y1, int x2, int y2)
{
  if (texflag)
...
void
tg_arrowline (int x1, int y1, int x2, int y2)
{
  if (texflag)
...
void
tg_settextjustify (int horiz, int vert)
{
  bgi_hj = horiz;
  bgi_vj = vert;
}
...
void
tg_outtextxy (int x, int y, char *textstring)
{
  int dx, dy;
...
void
tg_rectangle (int left, int top, int right, int bottom)
{
  int xx, yy;
  xx = bgi_x;
  yy = bgi_y;
...
void
tg_getlinesettings (struct tg_linesettingstype *sls)
{
...
void
start1 (char *window_name, char *icon_file, char *ini_file)
{
  if (!blind)
    {
      crt0_start (window_name, icon_file, ini_file, &nColors);
...
clr_eol (void)
{
  clrbox (xc + 1, yc + 1, maxCol (), yc + 1);
}

static void
print0 (int xc, int yc, int color, int bkcolor, char *s)
{
  char buff[STRSIZ];
  int i, x, y, h1, w1;
  if (xc / CdX > maxCol ())
...
void
get_text (int x1, int y1, int x2, int y2, void **dump)
{
  int dn;
  graphComPtr p, q;
...
put_text (void **dump)
{
  ...
  if (p == NULL)
    return;
  repeatGrCom (p->x1, p->y1, p->x2, p->y2, FALSE);
  ...
  while (p != NULL)
    {
      print0 (p->x1, p->y1, p->color, p->aux, p->text);
      addToGraphList (t_xt, p->x1, p->y1, p->x2, p->y2, p->color, p->aux,
		      p->text);
      q = p;
      p = p->next;
      free (q->text);
...
void
clrbox (int x1, int y1, int x2, int y2)
{
  repeatGrCom ((x1 - 1) * CdX, (y1 - 1) * CdY, x2 * CdX - 1, y2 * CdY - 1, FALSE);
  xc = x1 - 1;
  yc = y1 - 1;
}
...
inkey (void)
{
  ...
  if (!inkeyString)
      ...
      do
	{
      ...
      while ((key == KB_MOUSE) && mouse_info.but1 != 2);
    }
  else
    {
      key = inkeyString[0];
