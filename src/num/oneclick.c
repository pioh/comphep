...
static void f4_key_prog_num (int x) {
  char * command = scat ("%sbin/diag_viewer.exe&", pathtocomphep);
  system (command);
  return;
}

static void f5_key_prog_num (int x) {
  char * command = scat ("%sbin/diag_viewer2.exe&", pathtocomphep);
  system (command);
  return;
}
...
/*    LOCK = fopen ("LOCK", "a+");   FOR TESTS!!!!!!!!!!! */
    if (ftell (LOCK) == 0) {
      ...
      fprintf (stderr, "\noneclick (warning): program was already launched and not finished normally.\n\n");
