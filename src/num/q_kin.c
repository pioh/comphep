...
static double sqrt_S, ssmin, ssmax, stop, sbot, pcm;
...
double mkmom (double *x, double *ytilda) {
  int i, k, l;
  int nx;
  int iabc;
  int ns;
  ...
  if (nin_ == 2)
    {
      ...
      if (get_sf_num(0) || get_sf_num(1)) {
          ...
          if (bes < 1.) {
            ...
            final_factor *= ff / (al * fct2 / fct1 + (1 - al));
          } else {
            final_factor *= fct1;
          }
          ...
          if (get_sf_num(0) && get_sf_num(1))  {
            ...
            if (beta[0] < 1. && beta[1] < 1.) {
              ...
              final_factor *= pow (divy_ (y1), beta[0] - 1) * 
                        pow (divy_ (y2), beta[1] - 1) / (pow (psy1, 1 - beta[0]) 
                      + pow (psy2, 1 - beta[1]));

              if (bes < 1.) {
                final_factor *= pow (divy_ (yy), 1 - bes);
              } else {
                final_factor *= bes * pow (yy, bes - 1);
              }
            } else if (beta[0] < 1.) {
                psy1 = pow (xx, 1 / beta[0]);
                y1 = yy * psy1;
                y2 = yy * (1 - psy1);
                final_factor *= pow (yy, *beta) * pow (divy_ (y1), beta[0] - 1);
              }
            else if (beta[1] < 1.) {
              psy2 = pow (xx, 1 / beta[1]);
              y2 = yy * psy2;
              y1 = yy * (1 - psy2);
              final_factor *= pow (yy, beta[1]) * pow (divy_ (y2), beta[1] - 1);
            } else {
              y1 = yy * xx;
              y2 = yy - y1;
              final_factor *= yy;
...
/* *  FILLING ZERO COMPONENTS FOR in-PARTICLES */
  ...
  for (k = 0; k < nin_; ++k) {
    pvFill (pm[k], pIn[k], k + 1);
  }

  if (nin_ == 2) {
    final_factor /= 4 * pcmtilda * rstilda;
  } else {
    rstilda = pm[0];
    final_factor /= rstilda * 2;
    *ytilda = 0;
  }
  ...
  for (i = 0; i < nout1; ++i) {
    ...
    for (k = 0; k < 2; ++k) {
      if (kinmtc_1[i].lvout[k][1]) {
        ...
        dabc = k ? sval - amass[i][0] : sval - summas[i][1];

        smax = dabc * dabc;
        dabc = summas[i][k];
        smin = dabc * dabc;
        if (nmscut[i][k])
          rancor (&smin, &smax, 0., 1., nmscut[i][k] - 1);
  ...
  for (i = 0; i < nout1; ++i)   /*  MAIN CYCLE */
    {
      ...
      lvtonv (sph_inf[i][ns___].lvpole, nin_, nvpole);

      decay0 (nvin[i], amass[i][0], amass[i][1], &fct0);
      if (fct0 == 0.)
        {
          return 0.;
        }
      decay1 (nvpole, hsum, &hdif);
      ...
      for (k = 0; k < 2; ++k)
        {
          dabc = ((k << 1) - 1) / hdif;
          getreg_ (&nsing, singar, hsum[k], dabc, sph_inf[i][ns___].ncsreg[k]);
          rancor (&cosmin, &cosmax, hsum[k], dabc, sph_inf[i][ns___].ncscut[k] - 1);
          if (cosmin >= cosmax)
            {
              return 0.;
            }
      ...
      parfi = (xfi * 2 - 1) * M_PI;
      decay3 (nvposy, parcos, parfi, nvout[i][0], nvout[i][1]);
      ...
      for (ns = 0; ns < iabc; ++ns)
        if (ns != ns___)
          {
            lvtonv (sph_inf[i][ns].lvpole, nin_, nvpole);
            decay1 (nvpole, hsum, &hdif);
            decay2 (nvout[i][1], &parcos);
            cosmin = -1.;
            cosmax = 1.;
            nsing = 0;
            for (k = 0; k < 2; ++k)
              {
                dabc = ((k << 1) - 1) / hdif;
                getreg_ (&nsing, singar, hsum[k], dabc, sph_inf[i][ns].ncsreg[k]);
                rancor (&cosmin, &cosmax, hsum[k], dabc, sph_inf[i][ns].ncscut[k] - 1);
                if (cosmin >= parcos || parcos >= cosmax)
                  {
                    return 0.;
                  }
              }
            regfct_ (sph_inf[i][ns].itypep, nsing, singar, cosmin, cosmax,
                     parcos, &fct);
            fct_1__ += sph_inf[i][ns].sph_we / fct;
          }
      final_factor *= fct0 / fct_1__;
    }
  if (nin_ == 2)
    {
      if (get_sf_num(0))
...
int imkmom (void) {
  ...
  for (i = 0; i < nin_ + nout_; i++) {
    pinf_ (proces_1.nsub, i + 1, NULL, pm + i);
  }
...
/* *  NVOUT( , ) FILLING */
  ...
  if (nin_ == 2) {
    double m0 = get_sf_num(0) ? get_sf_mass(0) : pm[0];
    double m1 = get_sf_num(1) ? get_sf_mass(1) : pm[1];

    vinf_ (0, NULL, &sqrt_S);
    if (sqrt_S <= m0 + m1) {
      return 0;
    }
    pcm = cmfun (sqrt_S, m0, m1);
