...
# v  1.17 21.04.2009 1) bug fixed: negative session numbers can't be assigned properly (always n = -1)
#                       now, negative vaules can be used
...
# 2) default -> lhaef, cpyth1 -> -cpyth1, cpyth2 -> -cpyth2
# 3) time reports and forecast
# 4) -auto option 
# 5) divide help sections, a-la ./num_batch.pl help run 
...
GetOptions (
           ...
           "v",         \$vopt,                 # print out version of the script
...
# print the script version
  version() if (defined $veropt or defined $vopt);
...
###################################################################################################
# Launch n_comphep in the butch mode
  run () if defined $run;
  exit (0);
...
###################################################################################################
# priority rescheduling for n_comphep.exe
sub set_nice {
  @directories_to_search = ("/bin","/usr/bin","/sbin", "/usr/sbin");
  ...
  if ($nice ne "") {
    ...
    printf "$warn can't find nice, so priority rescheduling is not used\n";
  }
}
...
# replace sections #Initial_state, #Physical_Parameters, #Width_scheme, and #QCD
  ...
  while (<BATCH>) {
    ...
    $blocks[$number_cuts]      = $ses_blocks[$number_cuts];
...
~;
      ...
      if ($pid == 0) {
        exec("$b_prefix ./run.sh") or die "WARNING: Couldn't calculate Session $Nsession of process $proc_n[$i]: $!\n";
...
# Wait until all of the subprocesses to be calculated
  ...
  print "All of the subprocesses are finished \n";
...
sub list_batch {
  ...
  while (<BATCH>) {
    ...
    if ($#proc_n eq -1) {
      ...
      $show = 1 if grep { $Nproc_current eq $_ } @proc_n ;
    }
    if ($show eq 1) {
      ...
      if ($show_p{"cs"}) {
        if ($blocks[12] =~ /Vegas_integral\s+([-]*\d\.\d+E[+-]\d+)\s+([-]*\d\.\d+E[+-]\d+)\s+([-]*\d\.\d+E[+-]\d+)\s+(\d+)\s+(\d+)\s+(\d+)/) {
          ...
          printf "%s: cross section [pb] = %1.4e   +\/- %1.2e ( %1.2e %% )\n", $blocks[1], $cs, $cs_err * abs($cs), 100. * $cs_err;
        } else {
          printf "%s: Error! can't decode the Vegas_integral string...\n", $blocks[1];
        }
      }
    }
  }
...
sub list_mix {
 ...
 foreach (@ARGV) {
   ...
   print "List of subprocesses in mixed event file: $_ \n\n" ;
    $/="\n"; 
    while (<PRT>){if(/(#PROCESS|\#Cross_section|\#Number_of_subprocesses|\#Total_cross|\#Events_mixed)/g)
     {$_=~ s/\#// ; $_=~ s/\s{3,4}/ /g ;
      $_=~ s/Cross_section\(Width\)/    Cross_section/g ;print ;}
...
-nice nn           - run CompHEP with rescheduled priority (nice), nn - is new priority (1-19), 
...
###################################################################################################
sub version {
  print "$0 (CompHEP numerial tool) version $version\n";
  print "Last release date: $datestamp\n";
...
###################################################################################################
# auxiliary routine for set_nice
sub wanted
{
  if($_ eq "nice") {
   $nice = $File::Find::dir."/".$_;
   return;
  }
}
